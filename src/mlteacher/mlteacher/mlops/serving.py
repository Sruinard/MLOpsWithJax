...
def save_model(model, params, ctable, serving_dir):
    ...
    signatures[tf.saved_model.DEFAULT_SERVING_SIGNATURE_DEF_KEY] = tf_graph.get_concrete_function(serving_spec[0], serving_spec[1])

    latest_model_subpath = datetime.now().strftime("%Y%m%d%H%M%S")
    tf_model = _SavedModelWrapper(tf_graph, param_vars)
    model_serving_path = os.path.join(serving_dir, latest_model_subpath)
